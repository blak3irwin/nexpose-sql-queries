...
GROUP BY da.asset_id, da.ip_address, da.host_name, da.mac_address, dv.title, dv.date_added, favf.date, dta.tag_id, dv.riskscore
...
SELECT SUM(total_count)as TOTAL_COUNT, SUM(total_count) - SUM(high_to_urgent_count) AS LOW_MED_ONLY_COUNT, round((SUM(total_count) - SUM(high_to_urgent_count)) / SUM(total_count),4) AS PERCENTAGE_ONLY_LOW_MED
FROM STEP_2
